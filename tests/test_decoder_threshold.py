"""
Threshold analysis tests for BP+OSD decoder.

This module contains pytest tests for generating threshold plots
across different code distances and error rates, including comparison
with the ldpc library.
...
# Configuration
DISTANCES = [3, 5, 7]  # Reduced for memory constraints
ERROR_RATES = [0.0005, 0.0007, 0.001, 0.0015, 0.002]
ITER = 20
SAMPLE_SIZE = 2000  # Reduced for memory constraints

def run_bpdecoderplus_gpu_batch(H, syndromes, observables, obs_flip, priors,
                                 osd_order=10, max_iter=ITER, chunk_size=500):
    """
...
# cpu test
# def _decode_single_dataset(args):
#     """
#     Worker function for parallel CPU decoding of a single dataset.

#     Args:
#         args: Tuple of (distance, error_rate, osd_order, max_samples)

#     Returns:
#         Tuple of (distance, error_rate, ler) or None if dataset not found
#     """
#     distance, error_rate, osd_order, max_samples = args
...
#     marginals = bp_decoder.decode(batch_syndromes, max_iter=ITER, damping=0.2)
...
#     return (distance, error_rate, ler, num_samples)
...
def collect_threshold_data_parallel(osd_order: int = 10, max_samples: int = SAMPLE_SIZE, num_workers: int = None):
    """
    Collect logical error rates for threshold analysis using CPU parallel processing.
    ...
    if num_workers is None:
        num_workers = min(multiprocessing.cpu_count(), len(DISTANCES) * len(ERROR_RATES))

    # Prepare all tasks
    tasks = []
    for d in DISTANCES:
        for p in ERROR_RATES:
            tasks.append((d, p, osd_order, max_samples))
    ...
    with ProcessPoolExecutor(max_workers=num_workers) as executor:
        futures = {executor.submit(_decode_single_dataset, task): task for task in tasks}

        for future in as_completed(futures):
            result = future.result()
            if result is not None:
                d, p, ler, num_samples = result
                results[d][p] = ler
                print(f"  d={d}, p={p}: LER={ler:.4f} ({num_samples} samples)")
...
def plot_threshold_overlay(bp_results: dict, ldpc_results: dict,
    ...
    plt.yscale('log')
    plt.xscale('log')
    plt.axvline(x=0.01, color='gray', linestyle='--', alpha=0.5, label='p=1%')
...
class TestThresholdAnalysis:
    """Threshold curve analysis with optional ldpc comparison."""

    def test_threshold_plot(self):
        """
        Generate threshold plots for d=3,5,7 using GPU batch processing.

        This test collects BPDecoderPlus data and generates the main threshold plot.
        ...
        assert bp_points > 0, "No threshold data collected - generate datasets first"
        ...
        assert Path("outputs/threshold_plot.png").exists(), "Threshold plot was not created"
        ...
        if LDPC_AVAILABLE:
            ...
            if ldpc_points > 0:
                ...
                assert Path("outputs/threshold_plot_ldpc.png").exists(), "ldpc threshold plot was not created"
                ...
                assert Path("outputs/threshold_comparison.png").exists(), "Comparison plot was not created"
                assert Path("outputs/threshold_overlay.png").exists(), "Overlay plot was not created"
    ...
    ##cpu test
    # def test_threshold_plot_parallel(self):
    #     """Generate threshold plot using CPU parallel processing."""
    #     print("\nCollecting threshold data (CPU parallel mode)...")
    #     results = collect_threshold_data_parallel(osd_order=10, max_samples=SAMPLE_SIZE)

    #     # Check we have at least some data
    #     total_points = sum(len(v) for v in results.values())
    #     assert total_points > 0, "No threshold data collected - generate datasets first"

    #     print(f"\nCollected {total_points} data points")

    #     # Generate plot
    #     plot_threshold_curve(results, "outputs/threshold_plot_parallel.png")

    #     # Verify plot was created
    #     assert Path("outputs/threshold_plot_parallel.png").exists(), "Threshold plot was not created"
